...
template<typename Child>
template<typename Function> const Child * BaseList<Child>::forOne(const Function & f) const
    {
    const BaseList * item = this;
    const BaseList * next;
        ...
        next = item->next_ptr;
		item = next->next_ptr;
		bool a = f(static_cast<const Child*>(next));
    return static_cast<const Child*>(next);
    }
template<typename Child>
