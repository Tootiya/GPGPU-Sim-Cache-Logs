...
namespace jw
    ...
    typedef std::vector< std::pair< uint64_t, std::string > > aVector;
    ...
    class Sequitur
        ...
        //print things:
        void printList(const Symbol *, unsigned int number, std::ofstream &fout) const;
		void countAndMarkTerminals(Symbol *, unsigned int number, unsigned &n);
		void countAndMarkNew(Symbol *, unsigned int number, unsigned &n);
		void countHead(const Symbol *, unsigned int number, unsigned &n) const;
        void markSymbols2(Symbol * list, std::string action, aVector &v, std::ofstream &fout);
        //void markHead(Symbol *, bool mark);
        bool inSeenVector(uint64_t i);
        void printAll() const;
		unsigned int findTerminals();
		unsigned int findNew(unsigned int rc);
		unsigned int findHead(unsigned int rc) const;
        void markSymbols(aVector &v, std::ofstream &fout);
        void printSequence() const;
        void printRules(std::ofstream &fout) const;
        ...
        std::vector<uint64_t> seenVector;
        aVector theVector;
        };
	...
	template<typename Type>
	void Sequitur<Type>::countAndMarkTerminals(Symbol * list, unsigned int number, unsigned &n)
	{
		list->forUntil([&number, &n, this](Symbol * item)
		{
			if (typeid(*item) == ValueType)
			{
				++n;
                item->setMyType(1); // nonrep
			}
	...
	template<typename Type>
	void Sequitur<Type>::countAndMarkNew(Symbol * list, unsigned int number, unsigned &n)
	{
		list->forUntil([&number, &n, this](Symbol * item)
		{
			if (typeid(*item) == ValueType)
	...
	template<typename Type>
	void Sequitur<Type>::countHead(const Symbol * list, unsigned int number, unsigned &n) const
	{
		list->forUntil([&number, &n, this](const Symbol * item)
		{
			if (typeid(*item) == RuleHeadType)
			{
				n = n + static_cast<const RuleHead*>(item)->getCount() - 1;
    		}
    ...
    template<typename Type>
    bool Sequitur<Type>::inSeenVector(uint64_t num)
    {
        for (unsigned int i = 0; i < seenVector.size(); ++i) {
            if (num == seenVector[i])
                return true;
        }
        return false;
    ...
    /*template<typename Type>
    void Sequitur<Type>::markHead(Symbol * list, bool mark)
    {
        while (list) {
            if (typeid(*list) == RuleSymbolType) {
                auto rule = static_cast<RuleSymbol*>(list);
                uint64_t theID = rule->getID();
                if (!inSeenVector(theID)) {
                    seenVector.push_back(theID);
                }
                else {
                    markHead(rule_index.at(theID), true);
                    list = list->next();
                    continue;
                }
                markHead(rule_index.at(theID), false );
            }
            else if (typeid(*list) == ValueType) {
                auto value = static_cast<ValueSymbol<uint64_t>*>(list);
                if (mark) {
                    list->setMyType(3); // head
                    return;
                }
            }
            list = list->next();
    ...
    template<typename Type>
    void Sequitur<Type>::markSymbols2(Symbol * list, std::string action, aVector &v, std::ofstream &fout)
    {
        while (list) {
            if (typeid(*list) == RuleSymbolType) {
                auto rule = static_cast<RuleSymbol*>(list);
                ...
                if (!inSeenVector(theID)) {
                    seenVector.push_back(theID);
                    markSymbols2(rule_index.at(theID), "markNew", v, fout);
                }
                else {
                    if (action == "markOpportunity") {
                        markSymbols2(rule_index.at(theID), "markOpportunity", v, fout);
                    }
                    else { 
                        markSymbols2(rule_index.at(theID), "markHead", v, fout);
                    }
                }
                ...
                auto value = static_cast<ValueSymbol<uint64_t>*>(list);
                if (action == "markNew") {
                    fout << value->getValue() << " NW\n";
                    v.push_back(std::make_pair(value->getValue(), "NW"));
                } else if (action == "markHead") {
                    fout << value->getValue() << " HD\n";
                    v.push_back(std::make_pair(value->getValue(), "HD"));
                    action = "markOpportunity";
                }
                else if (action == "markOpportunity") {
                    fout << value->getValue() << " OP\n";
                    v.push_back(std::make_pair(value->getValue(), "OP"));
                } else {
                    fout << value->getValue() << " NR\n";
                    v.push_back(std::make_pair(value->getValue(), "NR"));
                }
            }
            list = list->next();
        }
    ...
    template<typename Type>
    void Sequitur<Type>::printList(const Symbol * list, unsigned int number, std::ofstream &fout) const
        {
        ...
        list->forUntil([&](const Symbol * item)
            ...
            if(typeid(*item) == ValueType)
                ...
                {
				fout << "[" << static_cast<const RuleSymbol*>(item)->getID() << "] ";
                }
            else if(typeid(*item) == RuleHeadType)
                {
				fout << "[" << static_cast<const RuleHead*>(item)->getID()
    ...
    template<typename Type>
    void Sequitur<Type>::printRules(std::ofstream &fout) const
        {
        for(const auto & rule_pair : rule_index )
            {
			fout << rule_pair.first << ": ";
            printList(rule_pair.second, 0, fout);
			fout << std::endl;
            }
        }

	template<typename Type>
	unsigned Sequitur<Type>::findTerminals()
	{
		unsigned n = 0;
		countAndMarkTerminals(rule_index.at(0), 0, n);
		return n;
	}
	template<typename Type>
	unsigned Sequitur<Type>::findNew(unsigned int rc)
	{
		unsigned n = 0;
		//print out rules:
		auto rule_pair = rule_index.begin();
		while (rule_pair != rule_index.end()) {
			if (rule_pair->first == 0) {
				rule_pair++;
				continue;
			}
			countAndMarkNew(rule_pair->second, 0, n);
			rule_pair++;
		}
	...
	template<typename Type>
	unsigned Sequitur<Type>::findHead(unsigned int rc) const
	{
		unsigned n = 0;
		//print out rules:
		auto rule_pair = rule_index.begin();
		while (rule_pair != rule_index.end()) {
			...
			countHead(rule_pair->second, 0, n);
			rule_pair++;
		}
		return n;
	}
    template<typename Type>
    void Sequitur<Type>::markSymbols(aVector &v, std::ofstream &fout)
    {
        //print out rules:
        //auto rule_pair = rule_index.begin();
        fout << std::endl;
        markSymbols2(rule_index.at(0), "false", v, fout);
    }
    ...
    const Symbol * Sequitur<Type>::SequiturIter<ChildIter>::resolveForward(const Symbol * in)
        ...
        if(type == parent->ValueType)
            {
            output = in;
            //cout << "pointer_stack size = " << pointer_stack.size() << "\n";
            }
